...
function PathsModule.init()
  ...
  modules.game_interface.addMenuHook("pathing", tr("Add Path"), 
    function(menuPosition, lookThing, useThing, creatureThing)
      ...
      local gamemap = modules.game_interface.getRootPanel():recursiveGetChildByPos(mousePosition, false)
      if gamemap and gamemap:getClassName() == 'UIGameMap' then
        AutoPath.addPath(gamemap:getPosition(menuPosition))
      end
    end,
    function(menuPosition, lookThing, useThing, creatureThing)
...
function PathsModule.addPath(pos)
  AutoPath.addPath(pos)
end

function PathsModule.updatePathsList(positions)
  local list = Panel:getChildById('PathList')
  list:destroyChildren()

  for i, pos in ipairs(positions) do
    local label = g_ui.createWidget('Label', list)
    label:setTextAlign(AlignCenter)
    label:setText(string.format('(x: %4d y: %4d z: %2d)', pos.x, pos.y, pos.z))
  end
end

function PathsModule.clearPathsList()
  AutoPath.clear()
end
